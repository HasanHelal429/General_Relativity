...
- [x] **Phase 7 — Frame dragging (`Frame_Dragging.ipynb`)** ✅ done. Self-contained (no new `.py` module -- nothing else in this project needed Christoffel symbols before now, so they're derived fresh in the notebook via `sympy`, symbolically, for the full general-`theta` Kerr metric, then restricted to `theta=pi/2`). Confirmed by direct symbolic differentiation (not just argued) that every `Gamma^theta_{ab}` for `a,b` in `{t,r,phi}` vanishes exactly at the equator (Kerr's `theta->pi-theta` symmetry), so a gyroscope launched with zero polar spin stays *exactly* confined to the `(t,r,phi)` sector -- an exact reduction, not an approximation. `sympy` also independently reproduced the 8 nonzero `(t,r,phi)`-sector Christoffels I'd separately derived by hand from the metric's t-phi/r block-diagonal structure, term for term.

  Along a **circular** orbit every Christoffel is evaluated at fixed `r`, so parallel transport `dS^mu/dtau = -Gamma^mu_ab U^a S^b` reduces to a *linear, constant-coefficient* system -- solved **exactly** via the matrix exponential (`S(T) = expm(-Lambda T) S(0)`, no truncation error at all), rather than an approximate ODE integration.

  **A real bug found and fixed**: the Christoffel symbols were first lambdified with `M` already substituted to this project's `M=1` unit convention, leaving only `(r, a)` as free arguments. That's invisible for every in-project validation (which all use `M=1`), but Section 5's Gravity-Probe-B check calls the same functions at Earth's actual `M_geo ~ 4.4e-3 meters` -- the metric functions correctly picked up the tiny `M_geo`, but the (still `M=1`-pinned) Christoffels didn't, producing a geodetic precession off by a factor of ~17,000 and tripping the notebook's own `assert`. Fixed by keeping `M` a free `sympy.lambdify` argument throughout and passing it explicitly everywhere. Caught immediately by the notebook's own closing assertion (`relative error vs. GPB's measured geodetic precession < 5%`) rather than silently -- a good example of why the sanity-check-first habit established since Phase 1 keeps paying off.

  **A second, non-bug precision issue found and fixed**: computing the precession angle via `arccos(dot(S0,ST))` catastrophically loses precision once the angle drops below `~1e-8` rad (GPB's actual angle is `~6e-9` rad/orbit, `cos(angle) = 1 - O(angle^2)` is indistinguishable from `1.0` in float64) -- it silently returned exactly `0`. Fixed with the cancellation-safe identity `|S_T-S_0|_g = 2 sin(angle/2)` (exact, since both are unit vectors), giving `angle = 2*arcsin(chord/2)`, accurate at any scale.

  **Validated four ways**: (a) internal consistency -- parallel transport exactly conserves `|S|^2` and `S.U` (`~1e-14` across a `(r,a)`-grid, i.e. machine precision); (b) two independent numerical methods (matrix exponential vs. adaptive `solve_ivp`) agree to `~1e-13`; (c) matches a standard closed-form Kerr circular-orbit precession formula *exactly* (`~1e-14`) at `a=0` (the unambiguous Schwarzschild geodetic-precession case), and to a small, well-explained residual at `a!=0` -- the residual grows with spin and shrinks with radius (roughly `O(a^2)`, `O(1/r^2)`), consistent with the closed form itself being a leading-order-in-spin approximation while this notebook's direct parallel transport is exact to all orders in `a`, not a flaw in the transport code (confirmed by (a) and (b) both being unaffected by spin); (d) plugging in Earth's actual mass/orbital-radius (GPB's real altitude) reproduces the measured geodetic precession to `0.6%` (`6.637` vs. the real `6.6` arcsec/year). The equatorial-orbit spin-orbit correction at Earth's actual (tiny) spin parameter came out to `-1.24%` of the geodetic term -- the same order of magnitude as GPB's real frame-dragging fraction (`0.59%` of its geodetic value), though explicitly *not* claimed as the same quantity: GPB flew a polar orbit specifically to isolate Lense-Thirring precession (along Earth's spin axis) from geodetic precession (in the orbital plane) as two orthogonal effects, while this notebook's equatorial-orbit spin-orbit term is a related but geometrically distinct quantity -- stated as an order-of-magnitude plausibility check, not a literal reproduction.
